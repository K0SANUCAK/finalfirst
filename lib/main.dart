...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<Game> getGame() async {
    Game game=Game(
      shortDescription: '', title: '', thumbnailUrl: '', id: '', genre: '',
    );
    await _firebaseFirestore.collection('games').doc('pvp').get().then((value) {
      game = Game(
        shortDescription: value['shortDescription'], title: value['title'], thumbnailUrl: value['thumbnailUrl'], id: value['value'], genre: value['genre'],
      );
    });
    return game;
  }
  Future<void> addGame({required Game game}) async {
    _firebaseFirestore.collection('games').doc("pvp").set(
      {
        'title': game.title,
